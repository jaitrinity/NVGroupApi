...
$locationSql = "SELECT `LocationId`,`Name` FROM `Location` where `Tenent_Id` = $tenentId ";
...
$roleArr = array();	
$rmArr = array();
if($searchType == "employee" || $searchType == "location"){
	$roleSql = "SELECT `RoleId`,`Role` FROM `Role` where `Tenent_Id` = $tenentId ";
	...
	$rmSql = "SELECT `EmpId`,`Name` FROM `Employees` where `RoleId` = 3 and `Active` = 1 and `Tenent_Id` = $tenentId ";
...
if($searchType == "location"){
	$siteSql = "SELECT `LocationId`, `Name`, `Site_Type` FROM `Location` where `LocationId` !=1 and `Tenent_Id` = $tenentId ";
	$siteQuery=mysqli_query($conn,$siteSql);
	while($siteRow = mysqli_fetch_assoc($siteQuery)){
		$locationId = $siteRow["LocationId"];
		$siteName = $siteRow["Name"];
		$siteType = $siteRow["Site_Type"];
		$json9 = array(
			'paramCode' => $locationId,
			'paramDesc' => $siteType.' - '.$siteName
		);
