...
$filterSql2 = "";
if($empIdd == "all"){
	// calculate all employee distance.
}
else{
	$filterSql = "and `EmpId` = '$empIdd' ";
	$filterSql1 = "and `Emp_Id` = '$empIdd' ";
	$filterSql2 = "and `Employee Code` = '$empIdd' ";
}

$sql = "SELECT `EmpId` FROM `Employees` WHERE Active = 1 and Tenent_Id = 1 ".$filterSql;

$delSql = "DELETE FROM `Distance` where `Visit_Date` = '$yesterdayDate' ".$filterSql1;
mysqli_query($conn,$delSql);
$delSql = "DELETE FROM `TA_DA_Report` where `Date` = '$yesterdayDate' ".$filterSql2;
mysqli_query($conn,$delSql);
...
$taDaResponse = CallAPI("GET","http://www.trinityapplab.in/NVGroup/taDaReport_dateEmp.php?yesterdayDate=$yesterdayDate&empId=$empIdd","");
$output -> taDaResponse = json_decode($taDaResponse);
echo json_encode($output);

// file_put_contents('/var/www/trinityapplab.in/html/NVGroup/log/distanceCalculatelog_'.date("Y").'.log', json_encode($output)."\n", FILE_APPEND);
