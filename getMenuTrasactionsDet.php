...
// $sql = "SELECT `Checkpoints`.`CheckpointId`, `Checkpoints`.`Description`, `TransactionDTL`.`Value`, `TransactionDTL`.`DependChkId`, `TransactionDTL`.`Lat_Long`, `TransactionDTL`.`Date_time`, `Checkpoints`.`Value` as cp_options, `Checkpoints`.`TypeId` FROM  `TransactionDTL` join `Checkpoints` on  `TransactionDTL`.`ChkId` = `Checkpoints`.`CheckpointId`  WHERE `TransactionDTL`.`ActivityId` = '$transactionId' order by `TransactionDTL`.`SRNo` ";
$sql = "SELECT c.`CheckpointId`, c.`Description`, (case when c.TypeId = 5 then (case when d.Watermark_Image is null then d.Value else d.Watermark_Image end) else d.`Value` end) as `Value`, d.`DependChkId`, d.`Lat_Long`, d.`Date_time`, c.`Value` as cp_options, c.`TypeId` FROM  TransactionDTL d join Checkpoints c on  d.`ChkId` = c.`CheckpointId`  WHERE d.`ActivityId` = '$transactionId' order by d.`SRNo`";
...
$auditDetList = [];
if($menuId == 289 || $menuId == 290 || $menuId == 291 || $menuId == 292 || $menuId == 293 || $menuId == 294 || $menuId == 295 || $menuId == 296){
	if($fourthActivityId != null)
...
function prepareStatusDet($conn, $transId, $cpId){
	...
	$sql = "SELECT c.`CheckpointId`, c.`Description`, (case when c.TypeId = 5 then (case when d.Watermark_Image is null then d.Value else d.Watermark_Image end) else d.`Value` end) as `Value`, d.`DependChkId`, d.`Lat_Long`, d.`Date_time`, c.`Value` as cp_options, c.`TypeId` FROM  `Checkpoints` c left join `TransactionDTL` d on  c.`CheckpointId` = d.`ChkId` and d.`ActivityId` = $transId  WHERE c.`CheckpointId` in ($cpId) ORDER BY FIELD(c.`CheckpointId`,$cpId) ";
