...
$approverSql = "SELECT mp.ActivityId,mp.MenuId,mp.LocationId,concat(l.Site_Type,' - ',l.Name) as Name,l.Site_Id,l.GeoCoordinates,m.Verifier,m.Approver,mp.Start,mp.End,m.Caption,m.Sub,m.Cat,m.Icons,m.CheckpointId,h.VerifierActivityId,m.GeoFence,h.WorkStartDatetime,h.WorkEndDatetime
				from Mapping mp
				...
				where mp.ActivityId !=0 and mp.Approver = '$empId'  and h.Status in ('Verified')
				and h.ApproverActivityId is null";	

$approverSql .= " AND date(mp.Start) <= date(now()) AND date(mp.End) >= date(now())";			
...
while($ap = mysqli_fetch_assoc($approverQuery)){
	...
	$apObj->latlong = str_replace("/", ",", $ap["GeoCoordinates"]);
...
$verifierSql = "SELECT mp.ActivityId, mp.MenuId, mp.LocationId,concat(l.Site_Type,' - ',l.Name) as Name,l.Site_Id,l.GeoCoordinates,m.Verifier,mp.Start,mp.End,
				m.Caption,m.Sub,m.Cat,m.Icons,m.CheckpointId, m.GeoFence, h.VerifierActivityId,h.Verify_Final_Submit,h.Nominal_Latlong,h.WorkStartDatetime,h.WorkEndDatetime
				from Mapping mp
				join TransactionHDR h on (mp.ActivityId = h.ActivityId)
				join Menu m on (m.MenuId = mp.MenuId)
				left join Location l on (mp.LocationId = l.LocationId)
				where mp.ActivityId !=0 and find_in_set('$empId',mp.Verifier) <> 0 and h.Status in ('Created')
				and (h.Verify_Final_Submit is null or h.Verify_Final_Submit = 'No') AND mp.Active = 1";	

$verifierSql .= " AND date(mp.Start) <= date(now()) AND date(mp.End) >= date(now())";			
...
while($v = mysqli_fetch_assoc($verifierQuery)){
	...
	$vObj->latlong = str_replace("/", ",", $v["GeoCoordinates"]);
...
$assignSql = "SELECT mp.MenuId,mp.LocationId,mp.Start,mp.End,mp.MappingId,concat(l.Site_Type,' - ',l.Name) as Name,l.Site_Id,l.GeoCoordinates,
...
while($row = mysqli_fetch_assoc($assignQuery)){
	...
	$assignObj->latlong = str_replace("/", ",", $row["GeoCoordinates"]);
	if($locationIdIndex >= 0 && ($menuLocationDistance != null && trim($menuLocationDistance,' ') != '')){
...
// 				join Menu m on (m.MenuId = mp.MenuId)
// 				left join Location l on (mp.LocationId = l.LocationId)
// 				where mp.ActivityId !=0 and mp.Third = '$empId' and h.Status in ('PTW_03')
...
// 				join Menu m on (m.MenuId = mp.MenuId)
// 				left join Location l on (mp.LocationId = l.LocationId)
// 				where mp.ActivityId !=0 and (find_in_set('$empId',mp.Fourth) <> 0 or find_in_set('$empId',mp.Fifth) <> 0) and h.Status in ('PTW_04')
...
// 				join Menu m on (m.MenuId = mp.MenuId)
// 				left join Location l on (mp.LocationId = l.LocationId)
// 				where mp.ActivityId !=0 and mp.Fifth = '$empId' and h.Status in ('PTW_05')
...
// 				join Menu m on (m.MenuId = mp.MenuId)
// 				left join Location l on (mp.LocationId = l.LocationId)
// 				where mp.ActivityId !=0 and find_in_set('$empId',mp.Sixth) <> 0 and h.Status = 'PTW_90'
