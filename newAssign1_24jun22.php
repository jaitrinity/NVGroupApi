...
while($row = mysqli_fetch_assoc($assignQuery)){
	...
	$ptw = "";
	$status = "";
	if($row["MenuId"] == 289 || $row["MenuId"] == 290 || $row["MenuId"] == 291 || $row["MenuId"] == 292 || $row["MenuId"] == 293 || $row["MenuId"] == 294 || $row["MenuId"] == 295 || $row["MenuId"] == 296){
...
$verifierSql = "Select mp.ActivityId,mp.MenuId,mp.LocationId,l.Name,l.Site_Id,l.GeoCoordinates,m.Verifier,mp.Start,mp.End,
				...
				left join Location l on (mp.LocationId = l.LocationId)
				where mp.ActivityId !=0 and find_in_set('$empId',mp.Verifier) <> 0 and h.Status in ('Created','PTW_01')
...
while($v = mysqli_fetch_assoc($verifierQuery)){
	...
	else if($v["MenuId"] == 289 || $v["MenuId"] == 290 || $v["MenuId"] == 291 || $v["MenuId"] == 292 || $v["MenuId"] == 293 || $v["MenuId"] == 294 || $v["MenuId"] == 295 || $v["MenuId"] == 296){
		$vObj->name = $v["Site_Id"];
...
$approverSql = "Select mp.ActivityId,mp.MenuId,mp.LocationId,l.Name,l.Site_Id,l.GeoCoordinates,m.Verifier,m.Approver,mp.Start,mp.End,
				...
				join Menu m on (m.MenuId = mp.MenuId)
				left join Location l on (mp.LocationId = l.LocationId)
				where mp.ActivityId !=0 and mp.Approver = '$empId' and h.Status in ('Verified','PTW_02')
...
while($ap = mysqli_fetch_assoc($approverQuery)){
	...
	$ptw = "";
	$status = "";
	if($ap["MenuId"] == 289 || $ap["MenuId"] == 290 || $ap["MenuId"] == 291 || $ap["MenuId"] == 292 || $ap["MenuId"] == 293 || $ap["MenuId"] == 294 || $ap["MenuId"] == 295 || $ap["MenuId"] == 296){
...
while($th = mysqli_fetch_assoc($thirdQuery)){
	...
	$ptw = "";
	$status = "";
	if($th["MenuId"] == 289 || $th["MenuId"] == 290 || $th["MenuId"] == 291 || $th["MenuId"] == 292 || $th["MenuId"] == 293 || $th["MenuId"] == 294 || $th["MenuId"] == 295 || $th["MenuId"] == 296){
...
while($fo = mysqli_fetch_assoc($fourthQuery)){
	...
	$ptw = "";
	$status = "";
	if($fo["MenuId"] == 289 || $fo["MenuId"] == 290 || $fo["MenuId"] == 291 || $fo["MenuId"] == 292 || $fo["MenuId"] == 293 || $fo["MenuId"] == 294 || $fo["MenuId"] == 295 || $fo["MenuId"] == 296){
	...
	while($apfcp = mysqli_fetch_assoc($apfilledcpQuery)){
		...
		if($apfcp['CheckpointId'] == 5522){
			$supervisorMobile = $apfcp['answer'];
		}
		else if($apfcp['CheckpointId'] == 5523){
	...
	while($apcp = mysqli_fetch_assoc($apapprovercpQuery)){
		...
		else if($apcp['CheckpointId'] == 5329){
			$apcpObj->Chkp_Id = $apcp['CheckpointId'];
			$apcpObj->editable = 0;
...
while($six = mysqli_fetch_assoc($sixthQuery)){
	...
	$ptw = "";
	$status = "";
	if($six["MenuId"] == 289 || $six["MenuId"] == 290 || $six["MenuId"] == 291 || $six["MenuId"] == 292 || $six["MenuId"] == 293 || $six["MenuId"] == 294 || $six["MenuId"] == 295 || $six["MenuId"] == 296){
