...
// file_put_contents('/var/www/trinityapplab.in/html/SpaceWorld/log/log_'.date("Y-m-d").'.log', date("Y-m-d H:i:s").' '.$json."\n", FILE_APPEND);
...
if ((strpos($mobiledatetime, 'AM') !== false) || (strpos($mobiledatetime, 'PM')) || (strpos($mobiledatetime, 'am') !== false) || (strpos($mobiledatetime, 'pm')))   {
 ...
 if($event == 'Submit'){
	...
	if($actId == null  && $actId == ''){
			...
			$activitySql = "Insert into Activity(DId,MappingId,EmpId,MenuId,LocationId,Event,GeoLocation,Distance,MobileDateTime,Tenent_Id)"
							." values ('$dId','$mapId','$empId','$mId','$lId','$event','$geolocation','$distance','$date1',$tenentId)";
			//echo $activitySql;
			...
			if($actId == null  && $actId == ''){
				...
				if(mysqli_query($conn,$insertInTransHdr)){
					...
					if($siteName != ''){
						...
						if($verifier_Role != null && $verifier_Role !=''){
							...
							$otherSql = "SELECT * FROM `Employees` where RoleId in (46,55,56) and Active = 1";
				...
				foreach($checklist as $k=>$v)
				{
					...
					if(count($chkp_idArray) > 1){
						...
						else if($chkp_id == 5270){
							$actName = $answer;
						}
						else if($chkp_id == 5538){
							$auditorStatus = $answer;
						}
						else if($chkp_id == 5495 || $chkp_id == 5554 || $chkp_id == 5555 || $chkp_id == 5556 || $chkp_id == 5557 || $chkp_id == 5558 || $chkp_id == 5559){
							$workStatus = $answer;
						}
						else if($chkp_id == 5344){
							$riskLevel = $answer;
						}
						else if($chkp_id == 5539){
							$rdhNdhStatus = $answer;
						}
					}	
				...
				if($riskLevel < 15){
					...
					$sixthEmpId = "";
					$otherSql = "SELECT * FROM `Employees` where RoleId in (55,56) and Active = 1";
					$otherResult = mysqli_query($conn,$otherSql);
					while ($otherRow = mysqli_fetch_assoc($otherResult)) {
		...
		if($actId != null && $actId != ''){
			...
			else if($thRow['Status'] == 'PTW_90' && $rdhNdhStatus == "Approve"){
				$updateTransHdrSql = "Update `TransactionHDR` set `Status` = 'PTW_04', `SixthActivityId` = '$activityId' where `ActivityId` = $actId";
				mysqli_query($conn,$updateTransHdrSql);
			}
			else if($thRow['Status'] == 'PTW_90' && $rdhNdhStatus == "Reject"){
				$updateTransHdrSql = "Update `TransactionHDR` set `Status` = 'PTW_99', `SixthActivityId` = '$activityId' where `ActivityId` = $actId";
				mysqli_query($conn,$updateTransHdrSql);
			}
			else if($thRow['Status'] == 'PTW_04' && $isAuditor && $auditorStatus == "No gap found"){
				$insertAudit = "INSERT INTO `PTWAudit`(`ActivityId`, `AuditActivityId`) VALUES ($actId,$activityId)";
				mysqli_query($conn,$insertAudit);
			}
			else if($thRow['Status'] == 'PTW_04' && $isAuditor && $auditorStatus == "Reject"){
				$updateTransHdrSql = "Update `TransactionHDR` set `Status` = 'PTW_99'  where `ActivityId` = $actId";
				mysqli_query($conn,$updateTransHdrSql);
				$insertAudit = "INSERT INTO `PTWAudit`(`ActivityId`, `AuditActivityId`) VALUES ($actId,$activityId)";
				mysqli_query($conn,$insertAudit);
			}
			else if($thRow['Status'] == 'PTW_04' && $isCloser){
				$updateTransHdrSql = "Update `TransactionHDR` set `Status` = 'PTW_05', `FourthActivityId` = '$activityId' where `ActivityId` = $actId";
				mysqli_query($conn,$updateTransHdrSql);
			}

			else if($thRow['Status'] == 'PTW_05'){
				$updateTransHdrSql = "Update `TransactionHDR` set `Status` = 'PTW_06', `FifthActivityId` = '$activityId' where `ActivityId` = $actId";
				mysqli_query($conn,$updateTransHdrSql);
			}
			

				
			if($thRow['Status'] == 'PTW_02'){
				$updateMappingSql = "Update `Mapping` set `Third` = '$empId' where `ActivityId` = $actId";
				mysqli_query($conn,$updateMappingSql);

			}
			// else if($thRow['Status'] == 'PTW_04'){
			// 	$selectMappingSql = "SELECT * FROM `Mapping` where ActivityId = $actId";
			// 	$selectMappingResult = mysqli_query($conn,$selectMappingSql);
			// 	$mappingRow=mysqli_fetch_array($selectMappingResult);
			// 	$verifierEmpId = $mappingRow["Verifier"];
			// 	$updateMappingSql = "Update `Mapping` set `Fifth` = '$verifierEmpId' where `ActivityId` = $actId";
			// 	mysqli_query($conn,$updateMappingSql);
			// }
		}
	}
